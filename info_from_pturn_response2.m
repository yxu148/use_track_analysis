...
for folder_index = 1 : length(x_cell)  % loop for each basedir folder
    ...
    mkdir(fullfile(basedir, 'results_criteria2'));  % create a overall results folder to contain general data
...
for j = 1 : length(figlocation_list)
    ...
    for i = 1 : length(fieldnames(larvae))  % loop for each larva in larvae
        ...
        response = [getfield(larvae, ['larva', num2str(i)], 'response2', 'blue'), getfield(larvae, ['larva', num2str(i)], 'response2', 'red'), getfield(larvae, ['larva', num2str(i)], 'response2', 'bluered')];
        switch response
            case '000'
...
savename = strcat(basedir_cell{1}, '\results_criteria2', '\data.mat');  % there should be only 1 basedir in basedir_cell
if isfile(savename)
...
mkdir(fullfile(basedir_cell{1}, 'results_criteria2', 'results_fig'));
savename = strcat(basedir_cell{1}, '\results_criteria2', '\results_fig', '\pturn_all');
...
switch criteria
    case 'pturn1-pturn_rest'
        ...
        savename = strcat(basedir_cell{1}, '\results_criteria2', '\results_fig', '\turn_criteria_1');
        savefig(gcf, savename);
    case 'pturn12-pturn_rest'
        xlabel('pturn(0<t<6s) - p(turn(6<t<15s)', 'FontSize', 20); ylabel('Count', 'FontSize', 20);
        savename = strcat(basedir_cell{1}, '\results_criteria2', '\results_fig', '\turn_criteria_2');
        savefig(gcf, savename);
    case 'pturn1-pturn345'
        xlabel('pturn(0<t<3s) - p(turn(6<t<15s)', 'FontSize', 20); ylabel('Count', 'FontSize', 20);
        savename = strcat(basedir_cell{1}, '\results_criteria2', '\results_fig', '\turn_criteria_3');
        savefig(gcf, savename);
end
...
ylim([ax(1), ax(2)], [-1, 4]);
sgtitle('Raw pturn');
savename = strcat(basedir_cell{1}, '\results_criteria2', '\results_fig', '\Dkl_larvaIndex_reverse');
savefig(gcf, savename);
...
title('PCC of pturn in red', 'FontSize', 24);
savename = strcat(basedir_cell{1}, '\results_criteria2', '\results_fig', '\pcc_pturn_red');
savefig(gcf, savename);
...
savename = strcat(basedir_cell{1}, '\results_criteria2');
