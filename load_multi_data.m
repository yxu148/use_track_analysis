...
t_stim_start = [0, 600, 1200, 1800];  % start time (s) of each intensity of stimulation
t_stim_end = [600, 1200, 1800, 2400];
...
create_larvae = false;  % create/edit structure larvae
save_data = false;  % save larvae to data.mat
...
for folder_index = 1 : length(x_cell)  % loop for each basedir folder
    ...
    for k = 1 : length(x)  % loop for each expt in the eset
        ...
        xdata = GQled1Val.xData;  % xdata of all Global Quantity ledVals should be the same
        ydata = GQled1Val.yData;  % LED intensity in PWM
        ydata(ydata==50) = 90; ydata(ydata==100) = 60;  % flip high and low values
        ydata = ydata + GQled2Val.yData;
        index1 = find(GQled2Val.yData==15, 1, 'first'); % find the index of the first 15 element of led1Val
        temp_led1Val_yData = GQled1Val.yData(1:3e4);  % only select the first part so it's easier to pick index2
        ...
        ydata(index1 : index2) = ydata(index1 : index2) + 70;  % this is to make the square wave fluctruate around a center quantity, use index to make the combined square wave cleaner
        temp_ydata = ydata(3e4:end);  % only select the last part so it's easier to find index3
        index3 = find(temp_ydata == 0, 1, 'first') + 3e4 - 1;  % the begining of dark
        ydata(index3 : end) = ydata(index3 - (length(ydata) - index3) : index3);  % give the constant 0 period a square wave right before dark to add ton&toff
...
%         figure; plot(xdata, ydata)
        ...
        if create_larvae
            ...
            savename = strcat(basedir,['\results', d(x(k)).name(end-16:end-4)], '\data.mat');
            clear larvae  % if firstly run, to clear the larvae of last expt
...
track_pturn =  false;  % save pturn of all tracks into structure tracks
track_info = false;  % save the temporal and spatial info about start/end of all tracks
stepsize = 0.1; binsize = 0.5;  % seconds
texpt = t_stim_end(end);  % used to plot v-texpt
plot_mean_speed_all_expt_t = false;
for folder_index = 1 : length(x_cell)  % loop for each basedir folder
...
%             end
    ...
    if plot_mean_speed_all_expt_t
        ...
        figure; hold on;
        yyaxis left
        pathObj = fill(x_tofill, y_tofill, 0.8*[1 1 1], 'LineStyle', 'none');  % no edges for the patch
        plot(x_ton, y_v, '-');
        ...
        close;
    end  % end plot_mean_speed_all_expt_t
