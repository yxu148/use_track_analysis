...
% writematrix(speed_all, strcat(basedir_cell{1}, '\results_new', '\speed.xlsx'));
...
load(savename, 'speed_all', 'speed_type');
...
savename = strcat(basedir, '\results_new', '\results_fig', '\speed_all');
savefig(gcf, savename);

% plot the matrix speed_run_all
figure;
imagesc(speed_run_all); 
xline([1, 150, 300], 'w', stim_color); yline(boundary_type + 0.5, 'w', name_type_cell);
ylabel('Index of larva'); set(gca,'XTick',[])
cbar = colorbar; cbar.Label.String = 'Run Speed (cm/min)';
mkdir(fullfile(basedir, '\results_new', 'results_fig'));
savename = strcat(basedir, '\results_new', '\results_fig', '\speed_run_all');
savefig(gcf, savename);
